...
adjmat <- adjinf(mmdtpdf[, 2], mmdtpdf[, 3], mindist = 5) # the matrix specifying adjacency 
...
write.csv(MJS_statistics, file = "MJS distance.csv", row.names = FALSE)
```

```{r}
# draw density curves by cluster
temp2 <- sign(temp)
temp2[teststat$childnodes[62][[1]]] <- 1
temp2[teststat$childnodes[56][[1]]] <- 2
temp2[teststat$childnodes[63][[1]]] <- 3
temp2[teststat$childnodes[57][[1]]] <- 4
temp2[teststat$childnodes[24][[1]]] <- 5
...
colcol = rep(c(2, 3, 4, 5, 6, 7, 8), 3)
atitle.cl<-"2000-2023"
clusthistd3(kk,colseq=temp2+1,atitle.cl,colcol,xlims = c(0,2), ylims = c(0,2.5))
```