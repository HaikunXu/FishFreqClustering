...
densmatx = matrix(0, nrow(mmdt), 430)
densmaty = matrix(0, nrow(mmdt), 430)
for(i in 1:nrow(mmdt)) {
  weightvec = t(mmdt[i, 4:(3 + nbins)])
  weightvec = weightvec / sum(weightvec)
  tempmmd = density(seq(0.21, 1.50, 0.01), weights = weightvec, bw = 0.04)
...
alydens.spatial23 <-
  hclust.regionsmm(
    ...
    adjmat = adjmat,
    rr = sign(mmdtpdf$Number) # rr is the weighting factor; equal weighting is used in this case
  )
