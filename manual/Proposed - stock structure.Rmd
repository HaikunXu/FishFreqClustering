...
alydens.spatial23 <-
  hclust.regionsmm(
    ...
    adjmat = adjmat,
    rr = sign(mmdfpdf_catch$Catch) # rr is the weighting factor; equal weighting is used in this case
  )
...
MJS_statistics <- cbind(1:nrow(teststat$statmat), teststat$statmat, round(catch_prop,2))
write.csv(MJS_statistics, file = "MJS distance (Stock Structure).csv", row.names = FALSE)
```

```{r}
# draw density curves by cluster
temp2 <- rep(1, 67)
temp2[teststat$childnodes[55][[1]]] <- 2
temp2[teststat$childnodes[64][[1]]] <- 3
temp2[teststat$childnodes[60][[1]]] <- 4
...
ggplot(data = cluster) +
  ...
  geom_text(aes(x = lon, y = lat, label = sign(Weight)),
            color = "white",
            size = 7,
            ) +
...
atitle.cl<-"2000-2023"
clusthistd3(kk=4,colseq=temp2+1,atitle.cl,colcol,xlims = c(0,2), ylims = c(0,2.5))
```