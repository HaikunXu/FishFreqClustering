...
LF <-
  LF.DEL[, c("year", "quarter", "lat", "lon", paste0("X", 1:201))] %>%
  group_by(lat, lon) %>% mutate(N = length(unique(paste0(year, "-", quarter)))) %>% filter(N > 3) # remove the cells with less than 4 quarters of data since 2000
...
alydens.spatial23 <-
  hclust.regionsmm(
    ...
    adjmat = adjmat,
    rr = sign(rrs) # rr is the weighting factor
  )
```
